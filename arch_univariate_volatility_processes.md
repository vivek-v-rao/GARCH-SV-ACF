# Univariate volatility (GARCH-family) processes in `arch` (Python)

This document summarizes the **volatility process equations** implemented in the **univariate** module of the Python `arch` package (v7.2.0).

Notation (common to all models):
...
\[
\sigma_t^2 = \sigma^2 \quad (\sigma^2>0)
\]

(Implemented as a single constant variance parameter.) citeturn17view0
...
Typical constraints used in estimation include \(\omega>0\) and \(\alpha_i\ge 0\). citeturn13view1turn17view0
...
where \(\lambda\) is the `power` argument. citeturn17view0

Special cases (as named in the docs/code):
...
In `arch` this class supports \(p,q \in \{0,1\}\) and a power parameter (default 2.0). The variance dynamics are

\[
h_t = \omega + \left[1-\beta L - \phi L (1-L)^d\right]\epsilon_t^2 + \beta h_{t-1}
\]

where \(L\) is the lag operator and \(d\) is the fractional differencing parameter. citeturn15view1

Estimation uses the ARCH(\(\infty\)) representation
...
When `power` is not 2, the same ARCH(\(\infty\)) representation is used, replacing \(\epsilon_t^2\) by \(|\epsilon_t|^{p}\), where \(p\) is the `power`. citeturn15view1
...
If `common_asym=True`, then all \(\gamma_i\) are restricted to have a common value. citeturn15view0
...
where the model is specified by a set of lag-block lengths \(\{l_i\}_{i=1}^m\). citeturn13view0
...
This is the standard exponentially weighted moving-average variance recursion. citeturn14view3
...
- If `unit_scale=False` (default), a scale parameter \(s>0\) is estimated and
...
- `arch` documentation (volatility processes): https://arch.readthedocs.io/en/stable/univariate/volatility.html
- Source for volatility process definitions (v7.2.0): https://arch.readthedocs.io/en/latest/_modules/arch/univariate/volatility.html
